  ...
  return (
    <div>
      <div className={'flexbox-container flexbox-container_align-start margin-bottom'}>
        {tokens.map((token) =>(<div className={'flexbox-container_max-growth'} >
          <Picture alt={token.id.toString()} />
          <div>{token.id}</div>
          <div>{token.collection.name} [ID {token.collection_id}]</div>
          <div><Text size={'s'} color={'grey-500'}>Owner: </Text><AccountLinkComponent value={token.owner} /></div>
        </div>))}
      </div>
