...
const useAccountStages = <T>(stages: InternalStage<T>[], accountAddress: string): useStagesReturn<T> => {
  const { accounts, signTx } = useAccounts();

  const action = useCallback((stageAction, txParams, options) => stageAction({ txParams, options }), []);
